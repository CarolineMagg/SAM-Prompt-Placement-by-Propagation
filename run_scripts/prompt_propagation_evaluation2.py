...
from evaluation.nnunet_evaluation_connector import computer_metrics_like_nnunet_from_arrays_surface

logging.basicConfig(level=logging.INFO)


def compute_metrics(reference_mask, prediction_mask, spacings, cls):
    ...
    blub = computer_metrics_like_nnunet_from_arrays_surface(reference_masks_cls, prediction_masks_cls, spacings)
    return {cls: blub}


if __name__ == '__main__':
    logging.info(" ")
    logging.info("Evaluation with HD95, ASSD")
    parser = argparse.ArgumentParser(description="Evaluation of SAM generated predictions with HD95, ASSD")
    ...
    for im_idx, im_name in enumerate(mask_list):
        ...
        with multiprocessing.get_context("spawn").Pool(4) as pool:
            results = pool.starmap(
                compute_metrics,
                list(zip([reference_masks] * num_proc, [prediction_masks] * num_proc,
                         [spacing] * num_proc, class_labels))
            )

        metrics_log[im_name] = {k: v for res in results for k, v in res.items()}

    with open(os.path.join(output_dir, f'{dataset}_log2.json'), "w") as file:
